...
def output_detections(input_path, output_dir, detections, n_classes):
    # VideoCapture を作成する。
    ...
    # VideoWriter を作成する。
    save_path = output_dir / f"{input_path.stem}.avi"
...
def main():
    ...
    for inputs, pad_infos in tqdm(dataloader, desc="infer"):
        ...
        with torch.no_grad():
            ...
            detections += [output_to_dataframe(x, class_names) for x in outputs]

    # 検出結果を出力する。
    args.output_dir.mkdir(exist_ok=True)
    output_detections(args.input_path, args.output_dir, detections, len(class_names))
